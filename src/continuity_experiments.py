...
from src.plotting import create_plot, PlotConfig
...
def load_model(config: ModelConfig) -> tuple[AutoTokenizer, AutoModelForCausalLM]:
    ...
    try:
        ...
        for i in range(torch.cuda.device_count()):
            logger.info(
                f"GPU {i}: {torch.cuda.get_device_properties(i).total_memory / 1024**3:.2f} GB"
            )
...
if __name__ == "__main__":
    ...
    for experiment_config in [MONTH_CONFIG]:
        for model_config in [GEMMA_2B_CONFIG]:
            logging.info("=" * 100)  # Separator for each experiment
            logging.info(f"New script run started at {datetime.now()}")
            logging.info(
                f"Running experiment '{experiment_config}' \n \n with model setup \n '{model_config}' \n"
            )
            start_time = time.time()

            run_experiment(
                experiment_config=experiment_config, model_config=model_config
            )

            minutes, seconds = get_minutes_and_seconds(time.time() - start_time)
            logger.info(
                f"\n \n     Done! 🥳 All in all it took {minutes} minutes and {seconds} seconds for the experiment '{experiment_config.name} \n'"
            )
